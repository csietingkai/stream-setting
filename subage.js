...
const getMessage = (language = 'tw') => {
	const pattern = {
		en: `$(eval is=$(urlfetch json https://bit.ly/3Dnpujc);i=is.find(x=>x.name=='$(user)');n=i?.nick||'$(user)';d=((Date.now()-i?.subDate)/(86400000));i?"$(n) has sub $(d) days!":"$(user) no sub (☍﹏⁰)")`,
		tw: `$(eval is=$(urlfetch json https://bit.ly/3Dnpujc);i=is.find(x=>x.name=='$(user)');n=i?.nick||'$(user)';d=((Date.now()-i?.subDate)/(86400000));i?"$(n)已訂閱$(d)天！":"$(user)沒訂閱(☍﹏⁰)")`
	}
